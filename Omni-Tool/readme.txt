...
- Unity Recovery
- Unity Cargo Unload (loaded vehicles and optional storage Quick Remove)
- Downgrade Buildings
...
It can also save fourteen complete sculpting bundles containing the operation, tile step, material, and Box/Round Brush shape. Use the terrain-tool icon next to the operation to add a bundle; its colored buttons sit below material favorites. Left-click to load and right-click to remove. Bundle contents remain visible on hover even when ordinary tooltips are disabled.
Bundle tooltips show the operation and material first. The saved tile step is displayed only for Raise and Lower, where it affects the brush.
Its material dropdown and favorites show the associated mined-product icon and localized name. Terrain-only names and visual variants have Omni-Tool translations, while unknown mod-added material names are left intact. Tile step only applies to Raise and Lower, so its control is disabled for Flatten and Paint.
...
6. In Vehicle Depot Orders mode, choose Mark for scrapping, Cancel scrapping, Replace vehicle, or Cancel replacing before selecting. Mark and Replace each have a separate optional exact vehicle-type filter: enable it, press the pick button, and click a vehicle in the world before making a large selection. Replace never targets a vehicle whose prototype is identical to the chosen replacement; different variants remain valid. Normal compatibility and depot requirements still apply.
   ...
   Unity Cargo Unload has separate Vehicle remove cargo and Storage quick remove Micro-HUD checkboxes. Vehicles start enabled and storages start disabled in each profile. Enable either or both. Storage Quick Remove uses the game's variable Unity cost, which increases with stored contents; selection details show vehicle cost, storage cost, and total before committing a wide swipe.
...
13. Terrain Sculpting raises, lowers, flattens, or paints the dragged terrain using operation, material, height-step, and Box/Round Brush controls. Tile step applies only to Raise and Lower. Material favorites switch a single ground type, while fourteen sculpting bundles recall all four controls. Its selected terrain material applies during every operation, so Raise can build mountains from gravel, ore, or any other listed material.
14. Terrain Eraser removes selected terrain above a remembered absolute target. Its precision range is -10 to 0; enable Expanded height range for -50 to +50. It does not raise terrain already below the target.
    Save up to twenty-one target heights per profile with the gold star. Left-click a numbered favorite to use it; right-click to remove it. Selecting a height outside -10 to 0 enables Expanded range automatically. The slider remains available for fine adjustment.
...
19. Right-click or press Escape to cancel or leave the tool. The optional global two-stage setting cancels an armed selection or drag first, then deactivates the tool but leaves the Omni-Belt open and idle; one more press closes the belt. With the setting off, cancellation keeps its original one-action behavior.
...
Hide Unequipped removes disabled cards from the editor without changing their settings or hidden relative order. Tool and profile cards have permanent native drag handles and show a live insertion position while dragged. The bottom-right grip resizes the wider settings window horizontally and vertically.

The Vehicle Tools profile contains Move Vehicles, Unity Quick Repair, Unity Recovery, Unity Cargo Unload, Scrap / Replace, and Assign Logistics Zone. The Building Tools profile contains Unity Quick Repair, Building Settings Brush, Downgrade Buildings, and Cancel Deconstruction, plus eligible storage cheat tools. The Terraforming profile contains Mine / Dump Designations, Terrain Sculpting, Terrain Eraser, Tree Painter, and Nature Sweeper; Remove Marked Trees is prepared but starts disabled, as do both storage tools in that profile. Vehicle/Logistics, Buildings, and Sandbox are separately visible per profile. Unity Quick Repair and Unity Cargo Unload permanently carry Vehicle and Buildings tags; both storage cheat tools permanently carry Sandbox and Buildings tags. A dual-tag card stays visible if either category is shown.
