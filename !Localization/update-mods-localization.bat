...
echo ================================================
echo   Проверка файлов на актуальность и обновление
echo ================================================
echo          Проверка запущена, ожидаем...
echo ================================================
echo.
...
REM Короткие варианты путей только для сообщений (сама копия идёт по полным путям)
set "SRC_SHORT=%~nx1"
set "DST_SHORT=%DST:C:\Users\www\AppData\Roaming\Captain of Industry\Mods\=%"

REM Проверка, что источник существует
if not exist "%SRC%" (
    echo [ОШИБКА]    Источник не найден: "%SRC_SHORT%"
    goto :eof
)

REM Если назначения нет - просто предупреждаем, ничего не копируем
if not exist "%DST%" (
    echo [ВНИМАНИЕ]  Назначение не найдено: "%DST_SHORT%"
    goto :eof
)
...
if "%RESULT%"=="NEWER" (
    echo [КОПИРУЮ]   "%SRC_SHORT%" -^> "%DST_SHORT%"
    copy /Y "%SRC%" "%DST%" >nul
)
